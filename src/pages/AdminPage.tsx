...
const AdminPage = () => {
  ...
  const fetchUsers = async () => {
    try {
      const res = await axiosLayer.get<UserInfo[]>('/admin/users');
      setUsers(res.data);
    } catch {
  ...
  const handleCreateUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await axiosLayer.post('/admin/users', {
  ...
  const handleRevoke = async (id: string) => {
    try {
      await axiosLayer.post(`/admin/users/${id}/revoke`);
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      await axiosLayer.delete(`/admin/users/${id}`);
          ...
          <Card className="flex flex-col border-border shadow-sm">
            <CardHeader className="flex flex-row items-center justify-between bg-muted/30 pb-4">
              ...
              <Button 
                ...
                className="bg-blue-600 hover:bg-blue-700 text-white dark:bg-blue-600 dark:hover:bg-blue-700 shadow-sm"
              >
                        ...
                        <TableRow key={config.id} className="hover:bg-muted/30 transition-colors">
                          ...
                          <TableCell className="py-4">
                            <Input
                              ...
                              className="focus-visible:ring-blue-500 max-w-md"
                            />
                          </TableCell>
                          <TableCell className="py-4">
                          ...
                          <TableRow key={u.id} className="hover:bg-muted/30 transition-colors">
                            <TableCell className="font-medium">
                              <div className="flex items-center gap-2">
                                <div className="h-8 w-8 rounded-full bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 flex items-center justify-center font-semibold text-xs">
                                  {u.username.substring(0, 2).toUpperCase()}
                                </div>
                                {u.username}
                                {u.username === currentUsername && (
                                  <span className="text-[10px] bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-400 px-2 py-0.5 rounded-full ml-2">
                                    You
                                  </span>
                                )}
                              </div>
