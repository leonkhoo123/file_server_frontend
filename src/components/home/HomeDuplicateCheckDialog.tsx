...
interface HomeDuplicateCheckDialogProps {
  isOpen: boolean;
...
const FileDetailView = ({ label, detail }: { label: string, detail: FileDetail }) => (
  <div className="flex-1 min-w-0 bg-background border rounded-sm p-2 relative">
    ...
    <div className="flex items-center space-x-3 pr-2">
      {detail.isDir ? (
        <Folder className="h-6 w-6 shrink-0 text-blue-500 fill-blue-500/20" />
      ) : (
...
export default function HomeDuplicateCheckDialog({
  isOpen,
  onOpenChange,
  onConfirm,
  onCancel,
